...
import {
  Modal as AntdModal, Form, Input, Button, Breadcrumb, Icon,
} from 'antd';
...
export function renderBreadcrumb(items = []) {
  ...
  return (
    <Breadcrumb separator=">">
      {items.map(it => (
        <BItem {...it} title={typeof it.title === 'string' ? it.title : ''}>
          {it.title}
        </BItem>
      ))}
    </Breadcrumb>
...
export function renderModal(
  ...
  const data = context.state[stateKey];
  if (!data) return null;
  const closeModal = () => context.setState({ [stateKey]: null });
  if (!modalProps.closeModal) modalProps.closeModal = closeModal;
  if (!ctorProps.closeModal) ctorProps.closeModal = closeModal;
  const className = modalProps.title ? modalProps.title : 'antd-no-title-modal';

  if (typeof data === 'object') ctorProps = { ...data, ...ctorProps };

  return (
    <Modal className={className} visible onCancel={closeModal} onOk={closeModal} {...modalProps}>
    ...
    .map(item => {
      ...
      if (item.required) {
        ...
        if (item.component.type === Input) {
          const whitespace = 'whitespace' in item ? item.whitespace : item.component.type === Input;
          props.rules.unshift({
            required: true,
            whitespace,
