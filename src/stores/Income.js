...
class Income {
    ...
    listLoading = false
    @observable
    totalShare = { title: '分成总额', value: '-' }
    @observable
    settled = { title: '已结算金额', value: '-' }
    @observable
    noSettle = { title: '未结算金额', value: '-' }
    @observable
    settling = { title: '结算中金额', value: '-' }

    @action
    getMyProfit({ pageNo = 1 }) {
        this.listLoading = true
        G.api
            .getMyProfit({ params: { pageNo, itemsPerPage: this.pageSize } })
            .then(data => {
