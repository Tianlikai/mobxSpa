...
class OrderForm extends Component {
        ...
        G.api.getOpenService(params).then(data => {
            if (!data) {
                ...
                orgServiceList.forEach(service => {
                    ...
                    let {type, laveNum, accountNum, name, outrideNum} = service
                    if (type === 1) {
                        purchasedService[0] = {
                            key: '1',
                            name,
                ...
                let totalPrice
                orgServiceEntityList && orgServiceEntityList.forEach(product => {
                    let {cid: key, name: value, price: unitPrice} = product
                    if (key === 8) {
                        totalPrice = Number(product.price)
                        productType.unshift({key: '' + key, value, unitPrice: Number(unitPrice)})
                    } else {
                        productType.push({key: '' + key, value, unitPrice: Number(unitPrice)})
                    }
                })
                this.setState({
    ...
    onSubmit(values) {
        ...
        let {unitPrice: price, totalPrice: payMoney, selectNumber: buyNum, productType, selectProductType, amount} = this.state
        ...
        if (selectProductType === '7') {
            ...
            data.type = 1 // 充值服务
            data.payMoney = amount.endsWith('.') ? amount.substr(0, amount.length - 1) : amount
    ...
    render() {
        ...
        const styleSpinner = {'display': 'block', 'width': '30px', margin: '4px auto'}
        const unitText = selectProductType === '1' ? UNIT[0] : UNIT[1]
        let selectValue = productType ? productType[0].value : '智能课堂（支持1对多/1对1）'
        ...
        let amountProps = this.state.amountError ? {help: '充值金额不能为"0"或空', validateStatus: 'error'} : null
        return (
            <div>
                {loading && <Spinner />}
                {!loading && <Form onSubmit={this.onSubmit}>
                    <FormItem className='orderForm-item' label='机构编号' {...formItemLayout}>
                        {getFieldDecorator('orgId', {
                            rules: [{required: true, validator: this.handleSearchOrgControl}],
                    ...
                    <FormItem className='orderForm-item-service' {...formItemLayout} label='开通服务'>
                        {purchasedService.map((service, i) => {
                            let {name, key, totalAccounts, accountBalance, outrideNum} = service
                            let outerText = outrideNum ? `; 已超出:${outrideNum})` : ')'
                            if (!key) return <div className={DEFAULT_CLASSES[i]}>{DAFAULT_TEXT[i]}</div>
                            return <div key={key} className={DEFAULT_CLASSES[i] + ' ' + 'own'}>
                                {name}
                                { totalAccounts && <span>{`(账号总数:${totalAccounts}; 账号余量:${accountBalance}${outerText}`}</span>}</div>
                        })}
                    </FormItem>
                    ...
                    {(selectProductType !== '7' && selectProductType !== '8') && <SelectNumberItem {...SelectNumberItemProps} />}
                    
                    {(selectProductType !== '7' && selectProductType !== '8') && <FormItem className='accounting-item' {...formItemLayout} label='单价'>
                        <span className='ant-form-text'>{`${unitPrice.toFixed(2)}${unitText}`}</span>
                    </FormItem>}

                    {selectProductType !== '7' && <FormItem className='accounting-item' {...formItemLayout} label='总价'>
                        {!countLoading && <span className='ant-form-text'>{`¥${totalPrice.toFixed(2)}`}</span>}
                        {countLoading && <span className='ant-form-text'><Spinner style={styleSpinner} /></span>}
                    </FormItem>}

                    {selectProductType === '7' && <FormItem {...amountProps} required='true' className='orderForm-item' label='充值金额' {...formItemLayout}>
                        <Input maxLength='10' onChange={this.validatorAmount} value={this.state.amount} placeholder='请输入充值金额' />
                    </FormItem>}
