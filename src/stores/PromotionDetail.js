...
class PromotionDetail {
    ...
    @observable detailListTotal = 0
    @observable pageSize = 10
    @observable listLoading = false
    @observable basicInformation = INFORMATION
    @observable dataOverview = DATA_OVERVIEW

    @action
    getPromotionDetail({id, pageNo = 1}) {
        this.listLoading = true
        G.api.getPromotionDetail({params: {promotionId: id, pageNo, itemsPerPage: this.pageSize}}).then(data => {
            this.listLoading = false
            this.detailListTotal = data.totalCourseOrderNumber
            Object.keys(this.basicInformation).forEach(key => {
                if (key === 'createdAt') {
                    this.basicInformation[key].value = data[key] ? moment(data[key]).format('YYYY-MM-DD HH:mm:ss') : '-'
                } else if (key === 'grade') {
                    const pos = GRADE.findIndex(grade => grade.value === data[key])
                    this.basicInformation[key].value = data[key] && pos >= 0 ? GRADE[pos].text : '-'
                } else {
            ...
            this.detailList = data.courseOrderlist.map((item) => {
                ...
                item.grade = item.grade && pos >= 0 ? GRADE[GRADE.findIndex(grade => grade.value === item.grade)].text : '-'
                item.key = item.id
                return item
            })
        })
