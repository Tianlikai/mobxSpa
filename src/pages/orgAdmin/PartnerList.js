...
class SearchForm extends Component {
    ...
    render() {
        ...
        let initialValue = this.props.initialValue || {name: undefined, state: ''}
                ...
                <Form layout='inline' onSubmit={this.onSubmit}>
                    <FormItem>
                        {getFieldDecorator('type', {
                            ...
                            <Select>
                                {Object.keys(ORG_STATE).map(key => <Option key={key} value={key}>{ORG_STATE[key]}</Option>)}
                            </Select>
                        )}
                    </FormItem>
                    <FormItem>
                        {getFieldDecorator('queryCond', {
                            ...
                            <Search style={{width: 270}} placeholder='搜索机构编号、机构名称、手机号码' />
                        )}
                    </FormItem>
                    <FormItem>
...
class PartnerList extends Component {
    ...
    toggleOrgState() {
        ...
        console.log(id)
        this.props.OrganizationStore.disableOrg(id, newState, this.closeModal.bind(this))
    }
    ...
    render() {
        const {organizationList, organizationListTotal, listLoading} = this.props.OrganizationStore
        ...
        return (
            <div className='list'>
                ...
                <Modal
                    ...
                    cancelText='取消'
                    okText='确认'>
                    <h3>确认{this.state.state === 0 ? '禁用' : '启用'}该账号？</h3>
                </Modal>
            </div>
        )
    }
