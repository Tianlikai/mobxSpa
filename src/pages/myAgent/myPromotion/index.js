...
class MyPromotion extends Component {
    ...
    get columns() {
        return [{
            ...
            width: 155,
            render: (text, record) => {
                const shareStyle = {width: 70, color: '#1574D4', marginRight: 5, cursor: 'pointer'}
                const detailStyle = {width: 70, color: '#1574D4', marginLeft: 5, cursor: 'pointer'}
                return (
                    <div className='operations-orgGo'>
                        <span style={shareStyle} onClick={() => this.handleOpenShareModal(record)}>立即分享</span>
                        <span style={detailStyle} onClick={() => this.redirectToDetail(record)}>查看详情</span>
                    </div>
                )
            }
        }]
