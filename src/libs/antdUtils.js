...
export function renderBreadcrumb(items = []) {
    ...
    return <Breadcrumb separator='>'>
        {items.map(it => <BItem {...it} title={typeof it.title === 'string' ? it.title : ''}>{it.title}</BItem>)}
    </Breadcrumb>
}

export function renderModal(context, stateKey, modalProps, ctor, ctorProps = {}, Modal = AntdModal) {
    ...
    return <Modal className={className} visible onCancel={closeModal} onOk={closeModal} {...modalProps}>
 ...
 */
export function renderForm(items, {data = {}, layout = defautFormItemLayout, formProps = {}, noWrap, noFooter, footer, submitText = '提交'} = {}, context = null) {
    context = context || this
    let {form: {getFieldDecorator}} = context.props
    layout = layout || {}
    items = items
            ...
            if (item.required) {
                ...
                if (item.component.type === Input) {
                    ...
                    props.rules.unshift({
                        required: true,
                        whitespace,
                        message: typeof item.required === 'string' ? item.required : '此字段不能为空'
                    })
                } else {
                    props.rules.unshift({
                        required: true,
    ...
    if (!noFooter && !footer) {
        ...
        let wrapperCol = layout.wrapperCol && layout.labelCol ? {
            span: layout.wrapperCol.span,
            offset: layout.labelCol.span
        } : null
        footer = <FormItem key='footer' wrapperCol={wrapperCol}>
            <Button type='primary' size='large' htmlType='submit' loading={loading}>{submitText + (loading ? '中...' : '')}</Button>
        </FormItem>
    }
    ...
    return <Form onSubmit={context.onSubmit} {...formProps}>
        {items}
        {footer}
    </Form>
}
...
export function createForm(options, antdFormOptions) {
    return Component => {
        return Form.create(antdFormOptions)(class extends Component {
            onSubmit(e) {
                e.preventDefault()
