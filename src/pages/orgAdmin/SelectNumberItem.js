...
class SelectNumberItem extends Component {
    ...
    handlePageChange = () => {
        ...
        if (selectProductType !== '1' && hadSystemService && selectProductType > hadSystemService) numberType = 'upgrade'
        // 曾经购买过 智能评测 或 作业系统
        if ((hadEvaluationService && selectProductType === '1') || (hadSystemService && selectProductType !== '1')) accountLabel = '购买更多账号'
        return {numberType, accountLabel}
    }
    handlePurchasedValidator = (rules, value, cb) => { // 自定义增购控制
        let r = /^\+?[1-9][0-9]*$/
        if (!r.test(value)) {
            cb('请输入正确的数量')
            return null
    ...
    handleUpgradeValidator = (rules, value, cb) => { // 自定义升级控制
        let r = /^(0|[1-9][0-9]*)$/
        if (!r.test(value)) {
            cb('请输入正确的数量')
            return null
    ...
    render() {
        const {
            ...
            formItemLayout} = this.props
        // 计算页面 显示 普通模块/增购模块/升级模块
        let {numberType, accountLabel} = this.handlePageChange()
        const commonProps = {selectNumber, handleAccounting, openCustomInput}
        return (
            <FormItem className='accounting-item-number' {...formItemLayout} label={accountLabel}>
                {numberType === 'purchased' && <div className='buy-account-number'>
                    <SelectButtonList
                    ...
                    <SelectButtonList
                        ...
                        {...commonProps}>
                        {customInputVisible && getFieldDecorator('customNumber', {
                            rules: [{required: true, validator: this.handlePurchasedValidator}],
                            validateTrigger: 'onBlur'
                        })(
                            <Input maxLength={10} placeholder='请输入自定义数量大于等于100' />
                        )}
                    </SelectButtonList>
                </div>}
                {numberType === 'upgrade' && <div className='buy-account-number'>
                    <SelectButtonList
                        data={designatedNumber[numberType]}
                        {...commonProps}>
                        {customInputVisible && getFieldDecorator('customNumber', {
                            rules: [{required: true, validator: this.handleUpgradeValidator}],
                            validateTrigger: 'onBlur'
                        })(
                            <Input maxLength={10} placeholder='请输入自定义数量' />
                        )}
                    </SelectButtonList>
                </div>}
            </FormItem>
