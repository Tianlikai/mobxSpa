...
class Orders {
    ...
    @observable orderListTotal = 0
    @observable orderLog = []
    @observable orderLogTotal = 0
    @observable pageSize = 10
    @observable orderDetail = null
    @observable listLoading = false

    @action
    getOrderList({name, pageNo = 1, listType = 1, state, startTime, endTime}) {
        this.listLoading = true
        G.api.getOrderList({params: {name, pageNo, listType, itemsPerPage: this.pageSize, state, startTime, endTime}}).then(data => {
    ...
    getOrderLog({name, type, pageNo = 1, startTime, endTime}) {
        ...
        const params = this.deleteEmptyValue({startTime, endTime, name, pageNo, itemsPerPage: this.pageSize, type})
        console.log(params)
    ...
    getOrdDetail(id, cb) {
        G.api.getOrderDetail({urlParams: {orderId: id}}).then(data => {
            Object.keys(data).forEach(key => {
                ...
                else if (key === 'price') data[key] = target === '9.9' ? target + UNIT[0] : target + UNIT[1]
