...
class OrderForm extends Component {
    onSubmit(values) {
        let {area: AREA, className, englishPress, englishTeacher, grade, mathPress, mathTeacher, school} = values
        const province = AREA[0] || ''
        const city = AREA[1] || ''
        const area = AREA[2] || ''
        const data = {
            province, city, area, className, englishPress, englishTeacher, grade, mathPress, mathTeacher, school
        }
    ...
    render() {
        ...
        return (
            <div className='orderForm-container'>
                {true && <Form onSubmit={this.onSubmit}>
                    ...
                    <FormItem className='orderForm-item specialE' label='英语版本' {...formItemRadioLayout}>
                        {getFieldDecorator('englishPress', {
                            ...
                            <RadioGroup>
                                {englishType.map(version => <Radio key={version.dictvalue} value={version.dictvalue}>{version.dicttext}</Radio>)}
                            </RadioGroup>
                        )}
                    </FormItem>
                    <FormItem {...tailFormItemLayout}>
                        <Button htmlType='submit' type='primary' size='big'>立即创建</Button>
                    </FormItem>
                </Form>}
            </div>
        )
