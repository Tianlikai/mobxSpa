                    ...
                    if (key === 'note') {
                        return <FormItem key={key} {...formItemLayout} className='orderDetail-item' label={label}>
                            {getFieldDecorator('orderNote', {
                                ...
                                <TextArea disabled={!this.props.edit} placeholder='请输入订单备注' rows={4} />
                            )}
                        </FormItem>
                    }
                    return <FormItem key={key} className='orderDetail-item' {...formItemLayout} label={label}>
                        <span className='ant-form-text'>{text}</span>
                    </FormItem>
                })}
                <FormItem className='orderDetail-item' {...tailFormItemLayout}>
                    <Button htmlType='submit' type={this.props.edit ? 'primary' : 'default'}>{this.props.edit ? '保存' : '编辑'}</Button>
                </FormItem>
            </Form>
        )
    }
...
export default class OrderDetail extends Component {
    ...
    componentDidMount() {
        ...
        this.props.OrdersStore.getOrdDetail(id, () => { this.setState({loading: false}) })
    }
    ...
    handleSubmit = (value) => {
        ...
        if (edit) {
            ...
            this.props.OrdersStore.editOrderNote(this.props.match.params.id, data, () => { this.setState({edit: !edit}) })
        } else {
